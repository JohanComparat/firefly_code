...
def writeScript(rootName, plate, env, sFlyList):
	...
	for el in sFlyList:
		...
		#print(el2)
		command = "python comply_to_data_model.py "+env+" "+plate+" "+el2[2]+" "+el2[3]+" \n"
		f.write(command)
...
for plate in plates:
	print(plate)
	rootName = join(os.environ['HOME'], "batch_data_model_compliance", plate)
	sFlyList = n.array(glob.glob(join(os.environ[env], 'stellarpop', plate, 'spFly-????-?????-????.fits')))
	#print(sFlyList)
