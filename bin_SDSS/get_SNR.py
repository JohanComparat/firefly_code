...
print(plate)
output_dir = join( os.environ['SDSSDR14_DIR'], 'SNR', str(plate).zfill(4))
...
fileList = np.array(glob.glob(join( os.environ['SDSSDR14_DIR'], 'spectra', str(plate).zfill(4), 'spec-*.fits')))
fileList.sort()
...
def runSpec(specLiteFile, output_dir):
	...
	snrs = spec.measure_SNR_SDSS_spectrum(survey='sdssMain')
