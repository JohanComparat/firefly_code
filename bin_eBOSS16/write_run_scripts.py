...
def writeScript(rootName, plate):
	...
	f.write(" \n")
	f.write("cd /users/comparat/firefly_code/bin_eBOSS16 \n")
	f.write("python run_stellarpop "+plate+" \n")
	f.write(" \n")
	f.close()


plates = n.loadtxt( join(os.environ['EBOSSDR16_DIR'], "catalogs", "plateNumberList"), unpack=True, dtype='str')
for plate in plates:
	rootName = join(os.environ['HOME'], "batch_dr16_v1", plate)
