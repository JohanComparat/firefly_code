...
4. creates a summary figure per plate - mjd combination
...
chi2 = n.ones((len(orig_table['MJD']),9))*dV
ndof = n.ones((len(orig_table['MJD']),9))*dV
...
def plot_spec_spFly(plate, mjd, fiberid, id_spec):
	"""
	...
	try :
		...
		if os.path.isfile(spFly_file) :
			...
			if len(cvm)>=1:
				...
				distMod = cosmo.distmod( obs[2].data[z_name] ).value 
				absolute_magnitudes[id_spec], arr = photo.computeMagnitudes(interp1d(wl_data, fl_data * 10. **(-17.)), distMod) 
				absolute_magnitudes_err[id_spec], arr = photo.computeMagnitudes(interp1d(wl_data, err_data * 10. **(-17.)), distMod) 
				#print("AbsMag", absolute_magnitudes[id_spec]) 
				#print("err", 10**-abs(absolute_magnitudes[id_spec]- absolute_magnitudes_err[id_spec])*10**(absolute_magnitudes[id_spec])) 
				...
				for j_hdu, hdu in enumerate(model[1:]):
					...
					if hdu.header['converged']=='True':
						...
						#if hdu.header['IMF']+"_"+hdu.header['library'] == "Chabrier_ELODIE" :
							#print(hdu.data['wavelength'])
							#absolute_magnitudes_cha_elodie[id_spec], arr = photo.computeMagnitudes(interp1d(hdu.data['wavelength'], hdu.data['firefly_model'] * 10. **(-17.)), distMod) 
							#print("AbsMag model", absolute_magnitudes_cha_elodie[id_spec]) 
...
for id_spec in range(len(orig_table['MJD'])):
	...
	plot_spec_spFly(plate, str(mjd).zfill(5), str(fiberid).zfill(4), id_spec)
...
table_all = n.ones(( len(orig_table['FIBERID']), 518)) * dV
headers = ""
for index, (fiber, mjd) in enumerate(zip(orig_table['FIBERID'], orig_table['MJD'])):
	fitFile = join( os.environ[env], dir, plate, "spFly-"+plate.zfill(4)+"-"+str(mjd)+"-"+str(fiber).zfill(4)+".fits")
...
files = os.path.join(os.environ[env], 'stellarpop', plate, 'spFlyPlate-????.fits')

splates = n.array(glob.glob(files))

def rewrite_plate_files(hd, mjd):
	path_2_out_file = os.path.join(os.path.dirname(splate), os.path.basename(splate)[:-5]+"-"+str(mjd).zfill(5)+".fits")
...
if len(splates)>0:	
	...
	print("plate split already done")
...
files = n.array(glob.glob(os.path.join(os.environ[env], 'stellarpop', plate, 'spFlyPlate-????-?????.fits')))
files.sort()

#print(files)
for path_2_out_file in files:
	print(path_2_out_file)
	im_file = os.path.basename(path_2_out_file)[:-5]+".png"
	path_2_im_file = os.path.join(im_dir, im_file)

	test = fits.open(path_2_out_file)
	#print(test[1].data.dtype)
	...
	bins = 10**n.arange(6,10,0.1)
	#print('age')
	for header in hdu_header_prefix :
		#print(header)
		conv = (test[1].data[header+'age_lightW'] != dV ) & (n.isnan(test[1].data[header+'age_lightW'])==False) & (test[1].data[header+'age_lightW'] != n.inf)
		#print("age", test[1].data[header+'age_lightW'][conv])
		nn_s, bb = n.histogram(test[1].data[header+'age_lightW'][conv], normed = True, bins=bins)
		xb = (bins[:-1]+bins[1:])/2.
		#p.plot(xb, nn_s, label=header[:-1]  , rasterized =True )
		p.hist( test[1].data[header+'age_lightW'][conv], normed = True, bins=bins, cumulative=True, rasterized =True, histtype='step', label=header[:-1] )
	...
	#print('mass')
	bins = 10**n.arange(8,12.5,0.1)
	for header in hdu_header_prefix :
		conv = (test[1].data[header+'age_lightW'] != dV )  & (n.isnan(test[1].data[header+'age_lightW'])==False) & (test[1].data[header+'age_lightW'] != n.inf) & (n.isnan(test[1].data[header+'stellar_mass'])==False) & (test[1].data[header+'stellar_mass'] != n.inf)
		#print("mass", test[1].data[header+'stellar_mass'][conv])
		nn_s, bb = n.histogram(test[1].data[header+'stellar_mass'][conv], normed = True, bins=bins)
		xb = (bins[:-1]+bins[1:])/2.
		#p.plot(xb, nn_s, label=header[:-1]  , rasterized =True )
		p.hist( test[1].data[header+'stellar_mass'][conv], normed = True, bins=bins, cumulative=True, rasterized =True, histtype='step' )
	...
	p.plot(test[1].data['Chabrier_ELODIE_stellar_mass'], test[1].data['Salpeter_ELODIE_stellar_mass'] / test[1].data[  'Chabrier_ELODIE_stellar_mass'], 'k+', rasterized = True, label='Salpeter/Chabrier')
	p.xlabel(r'$mass/[M_\odot]$ Chabrier')
	p.ylabel(r'mass ratio')
	p.grid()
	p.xscale('log')
	p.ylim((0,2))
	p.xlim((10**8,10**12.5))
	...
	p.plot(test[1].data['Chabrier_ELODIE_stellar_mass'], test[1].data['Kroupa_ELODIE_stellar_mass'] / test[1].data[  'Chabrier_ELODIE_stellar_mass'], 'k+', rasterized = True, label='Kroupa/Chabrier')
	p.xlabel(r'$mass/[M_\odot]$ Chabrier')
